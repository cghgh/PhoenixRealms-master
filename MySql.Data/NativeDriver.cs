  ...
  internal class NativeDriver : IDriver
  {
    ...
    internal void ReadOk(bool read)
    {
      try
      {
        ...
        var marker = (byte)packet.ReadByte();
        if (marker != 0)
        {
    ...
    private X509CertificateCollection GetClientCertificates()
    {
      ...
      if (Settings.CertificateFile != null)
      {
        ...
        var clientCert = new X509Certificate2(Settings.CertificateFile,
    ...
    private void SendFileToServer(string filename)
    {
      ...
      try
      {
            ...
            FileAccess.Read))
        {
          ...
          while (len > 0)
          {
            var count = fs.Read(buffer, 4, (int)(len > 8192 ? 8192 : len));
