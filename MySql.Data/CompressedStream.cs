...
namespace MySql.Data.MySqlClient
{
  ...
  internal class CompressedStream : Stream
  {
    ...
    public override int Read(byte[] buffer, int offset, int count)
    {
      ...
      var countToRead = Math.Min(count, maxInPos - inPos);
      int countRead;
      if (zInStream != null)
    ...
    private void PrepareNextPacket()
    {
      ...
      var compressedLength = lengthBytes[0] + (lengthBytes[1] << 8) + (lengthBytes[2] << 16);
      // lengthBytes[3] is seq
      var unCompressedLength = lengthBytes[4] + (lengthBytes[5] << 8) +
                   (lengthBytes[6] << 16);
      ...
      else
      {
        ...
        var ms = new MemoryStream(inBuffer);
    ...
    private MemoryStream CompressCache()
    {
      ...
      var zos = new ZOutputStream(compressedBuffer, zlibConst.Z_DEFAULT_COMPRESSION);
