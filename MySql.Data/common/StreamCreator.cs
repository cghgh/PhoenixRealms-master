...
namespace MySql.Data.Common
{
  ...
  internal class StreamCreator
  {
    ...
    private Stream GetStreamFromHost(string pipeName, string hostName, uint timeout)
    {
      ...
      {
        var ipHE = GetHostEntry(hostName);
        foreach (var address in ipHE.AddressList)
        {
          if (address.AddressFamily == AddressFamily.InterNetworkV6 && !driverVersion.isAtLeast(5, 5, 3))
            continue;
    ...
    public Stream GetStream(uint timeout)
    {
      ...
      var index = random.Next(dnsHosts.Length);
      var pos = 0;
      Stream stream = null;
    ...
    IPHostEntry GetDnsHostEntry(string hostname)
    {
      ...
      {
        var message = String.Format(Resources.GetHostEntryFailed,
    ...
    private static EndPoint CreateUnixEndPoint(string host)
    {
      ...
      var ep = (EndPoint)a.CreateInstance("Mono.Posix.UnixEndPoint",
