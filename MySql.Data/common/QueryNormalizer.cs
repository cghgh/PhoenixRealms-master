...
namespace MySql.Data.Common
{
  internal class QueryNormalizer
  {
    ...
    private bool ConsumeComment()
    {
      ...
      var startingIndex = pos;

      var index = fullSql.IndexOf(endingPattern, pos);
      if (index == -1)
        index = fullSql.Length - 1;
      else
        index += endingPattern.Length;
      var comment = fullSql.Substring(pos, index - pos);
    ...
    private void ConsumeUnquotedToken()
    {
      ...
      while (pos < fullSql.Length && !IsSpecialCharacter(fullSql[pos]))
        pos++;
      var word = fullSql.Substring(startPos, pos - startPos);
      double v;
