...
namespace MySql.Data.MySqlClient
{
  internal class MySqlTokenizer
  {
    ...
    public string NextToken()
    {
      while (FindToken())
      {
        var token = sql.Substring(startIndex, stopIndex - startIndex);
        return token;
      }
      return null;
    }
    ...
    public string NextParameter()
    {
      while (FindToken())
      {
        ...
        var c1 = sql[startIndex];
        var c2 = sql[startIndex + 1];
        if (c1 == '?' ||
            (c1 == '@' && c2 != '@'))
    ...
    private bool ReadComment(char c)
    {
      ...
      var startingIndex = pos - 1;

      var index = sql.IndexOf(endingPattern, pos);
      if (endingPattern == "\n")
        index = sql.IndexOf('\n', pos);
      if (index == -1)
        index = sql.Length - 1;
