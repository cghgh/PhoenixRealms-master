...
namespace MySql.Data.MySqlClient
{
  internal class ISSchemaProvider : SchemaProvider
  {
    ...
    private DataTable GetParametersFromIS(string[] restrictions, DataTable routines)
    {
      ...
      if (routines == null || routines.Rows.Count == 0)
      {
        if (restrictions == null)
        {
          ...
          var da = new MySqlDataAdapter("SELECT * FROM INFORMATION_SCHEMA.PARAMETERS WHERE 1=2", connection);
          da.Fill(parms);
        }
        else
    ...
    {
      var dt = new DataTable("Procedure Parameters");
    ...
    {
      var where = new StringBuilder(initial_where);
      if (values != null)
      {
        ...
        string[] keys, string[] values)
    {
      ...
      var where = GetWhereClause(initial_where, keys, values);
        ...
        string[] restrictions, DataTable routines)
    {
      ...
      {
        var showCreateSql = String.Format("SHOW CREATE {0} `{1}`.`{2}`",
