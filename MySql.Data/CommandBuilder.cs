...
namespace MySql.Data.MySqlClient
{
  ...
  public sealed class MySqlCommandBuilder : DbCommandBuilder
  {
    ...
    public static void DeriveParameters(MySqlCommand command)
    {
      ...
      try
      {
        var ds = command.Connection.ProcedureCache.GetProcedure(command.Connection, spName, null);
        var parameters = ds.Tables["Procedure Parameters"];
        var procTable = ds.Tables["Procedures"];
        ...
        foreach (DataRow row in parameters.Rows)
        {
          ...
          var real_as_float = procTable.Rows[0]["SQL_MODE"].ToString().IndexOf("REAL_AS_FLOAT") != -1;
