  ...
  public class MySqlScript
  {
    ...
    public int Execute()
    {
      ...
      try
      {
        ...
        var ansiQuotes = mode.IndexOf("ANSI_QUOTES") != -1;
        var noBackslashEscapes = mode.IndexOf("NO_BACKSLASH_ESCAPES") != -1;
    ...
    private void OnQueryExecuted(ScriptStatement statement)
    {
      ...
      {
        var args = new MySqlScriptEventArgs();
    ...
    private List<ScriptStatement> BreakIntoStatements(bool ansiQuotes, bool noBackslashEscapes)
    {
      ...
      var tokenizer = new MySqlTokenizer(query);
      ...
      while (token != null)
      {
        if (!tokenizer.Quoted)
        {
          ...
          else
          {
            ...
            var delimiterPos = token.IndexOf(currentDelimiter, StringComparison.InvariantCultureIgnoreCase);
            if (delimiterPos != -1)
            {
