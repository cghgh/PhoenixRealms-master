    ...
    public sealed class MySqlCommand : DbCommand, ICloneable
    {
        ...
        internal void ResetSqlSelectLimit()
        {
            ...
            if (resetSqlSelect)
            {
                ...
                var command = new MySqlCommand("SET SQL_SELECT_LIMIT=DEFAULT", connection);
        ...
        public new MySqlDataReader ExecuteReader(CommandBehavior behavior)
        {
            ...
            lock (driver)
            {
                ...
                try
                {
                    var reader = new MySqlDataReader(this, statement, behavior);
        ...
        {
            var clone = new MySqlCommand(cmdText, connection, curTransaction);
        ...
        internal string GetCommandTextForBatching()
        {
            if (batchableCommandText == null)
            {
                ...
                if (String.Compare(CommandText.Substring(0, 6), "INSERT", true) == 0)
                {
                    ...
                    var sql_mode = cmd.ExecuteScalar().ToString().ToUpper(CultureInfo.InvariantCulture);
                    ...
                    var token = tokenizer.NextToken().ToLower(CultureInfo.InvariantCulture);
                    while (token != null)
                    {
