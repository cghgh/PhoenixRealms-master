...
namespace MySql.Data.MySqlClient
{
  public class MySqlConnectionStringBuilder : DbConnectionStringBuilder
  {
    ...
    public bool IntegratedSecurity
    {
      get
      {
        var val = values["Integrated Security"];
        return (bool)val;
      }
      set
      {
    ...
    {
      ValidateKeyword(keyword);
      var primaryKey = validKeywords[keyword];
    ...
    private object ChangeType(object value, Type t)
    {
      ...
      {
        var s = value.ToString().ToLower(CultureInfo.InvariantCulture);
    ...
    {
      var key = keyword.ToLower(CultureInfo.InvariantCulture);
    ...
    {
      var properties = typeof(MySqlConnectionStringBuilder).GetProperties();
      foreach (var pi in properties)
        AddKeywordFromProperty(pi);

#if !CF
      // remove this starting with 6.4
      var encrypt = typeof(MySqlConnectionStringBuilder).GetProperty(
    ...
    {
      var name = pi.Name.ToLower(CultureInfo.InvariantCulture);
      ...
      foreach (Attribute a in attr)
      {
        ...
        {
          foreach (var keyword in (a as ValidKeywordsAttribute).Keywords)
