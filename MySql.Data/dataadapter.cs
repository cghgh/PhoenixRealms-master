...
namespace MySql.Data.MySqlClient
{
  ...
  public sealed class MySqlDataAdapter : DbDataAdapter, IDbDataAdapter, IDataAdapter, ICloneable
  {
    ...
    protected override int ExecuteBatch()
    {
      ...
      {
        var cmd = (MySqlCommand)commandBatch[index++];
        for (var index2 = index; index2 < commandBatch.Count; index2++, index++)
        {
          var cmd2 = (MySqlCommand)commandBatch[index2];
    ...
    protected override void ClearBatch()
    {
      ...
      {
        var cmd = (MySqlCommand)commandBatch[0];
        if (cmd.Batch != null)
          cmd.Batch.Clear();
      }
      commandBatch.Clear();
