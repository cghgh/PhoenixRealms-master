...
namespace terrain
{
    ...
    internal class PolygonMap
    {
        ...
        private void FindLakesAndCoasts()
        {
            ...
            var start = Polygons.First(_ => _.Nodes.Any(__ => __.X == -1 && __.Y == -1));
            lake.Remove(start);
        ...
        private void ComputeDistances()
        {
            ...
            {
                var n = queue.Dequeue();
                var dist = n.DistanceToCoast.Value;
                visited.Add(n);
                foreach (var i in n.Edges)
                {
                    var d = i.To.DistanceToCoast ?? double.MaxValue;
                    if (d > dist + 1)
                        d = dist + 1;
