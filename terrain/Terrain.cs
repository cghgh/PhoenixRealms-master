    ...
    internal class Terrain
    {
        ...
        private static void Show(IEnumerable<MapPolygon> polys, IEnumerable<MapNode> plot)
        {
            ...
            using (var g = Graphics.FromImage(map))
            {
                foreach (var poly in polys)
                {
                    ...
                    for (var j = 0; j < poly.Nodes.Length; j++)
                    {
                        var curr = poly.Nodes[j];
                        var prev = j == 0 ? poly.Nodes[poly.Nodes.Length - 1] : poly.Nodes[j - 1];
        ...
        private static int MinDistToMapEdge(PlanarGraph graph, Node n, int limit)
        {
            ...
            do
            {
                ...
                {
                    var node = graph.Find(i.DirectedCoordinate);
